...
def _preprocess(date, header, station, channel, component, remove_ir, client):
    ...
    if len(nonzero_data) > cfg.max_traces:
        print('More than {} gaps, skipping {} after {:.2f}s (load:{:.2f}s clean:{:.2f}s)!'.format(cfg.max_traces, date,
            (dt.datetime.now() - start_timer).total_seconds(), load_time, nonzero_time))
        return
    ...
    if percent_gap < cfg.gap_limit:
        ...
        print('Finished preprocessing for {} ({} traces) in {:.2f}s (load:{:.2f}s clean:{:.2f}s merge:{:.2f}s preprocess:{:.2f}s)'.format(date, len(nonzero_data),
            (dt.datetime.now() - start_timer).total_seconds(), load_time, nonzero_time, merge_time, preprocess_time))

    else:
        print('Too much of a gap: {}%, skipping {} after {:.2f}s (load:{:.2f}s clean:{:.2f}s merge:{:.2f}s)'.format(percent_gap, date,
            (dt.datetime.now() - start_timer).total_seconds(), load_time, nonzero_time, merge_time))
